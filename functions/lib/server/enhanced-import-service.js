...
import { db } from "./db";
import { importSessions, fieldMappingCache, } from "@shared/schema";
import { parse } from "csv-parse/sync";
import * as XLSX from "xlsx";
import { v4 as uuidv4 } from "uuid";
import { eq } from "drizzle-orm";
import { WorkflowOrchestrator, WorkflowState, } from "./services/workflow-orchestrator";
...
export class EnhancedImportService {
    ...
    async analyzeFile(req, res) {
        try {
            ...
            if (!file.path) {
                throw new Error("File path is missing - multer might be using memory storage instead of disk storage");
            }
            let fileBuffer;
            try {
                ...
                throw new Error(`Failed to read file from path ${file.path}: ${fsError.message}`);
            }
            // Use adaptive field integration for enhanced CSV parsing
            const adaptiveResult = await adaptiveFieldIntegration.extractFieldsWithAdaptiveParsing(fileBuffer, file.originalname, {
                maxSampleSize: 100,
            ...
            console.log("Adaptive field extraction completed:", {
                adaptiveParsingUsed: adaptiveResult.integrationMetadata.adaptiveParsingUsed,
                fallbackToBasic: adaptiveResult.integrationMetadata.fallbackToBasic,
                confidenceImprovement: adaptiveResult.integrationMetadata.confidenceImprovement,
            ...
            try {
                ...
                console.error("[ENHANCED IMPORT] Workflow automation error:", workflowError);
    ...
    async overrideFieldMappings(req, res) {
        try {
            const { sessionId } = req.params;
    ...
    async generatePreview(req, res) {
        try {
            ...
            const mappedData = await this.applyFieldMappings(fileData, session.fieldMappings);
            // Validate data
    ...
    async generatePreviewInternal(sessionId) {
        try {
            const session = await this.getSession(sessionId);
            if (!session) {
            ...
            console.log(`[ENHANCED IMPORT] Internal preview generated for session ${sessionId}: ${result.validRecords}/${result.totalRecords} valid records`);
            return result;
        }
        catch (error) {
            console.error("[ENHANCED IMPORT] Internal preview generation error:", error);
            return {
                success: false,
                sessionId,
                totalRecords: 0,
    ...
    async applyFieldMappings(data, mappings) {
    ...
    async getCachedFileData(sessionId) {
        try {
            ...
            console.log(`Successfully retrieved cached file data: ${data.length} records`);
            return data;
        }
        catch (error) {
    ...
    async processBatchImport(sessionId, importConfig) {
        try {
            // Get session data
            const session = await this.getSession(sessionId);
            if (!session) {
            ...
            await batchProcessor.processBulkImport(sessionId, mappedData, mappings, importConfig.entityType || "product");
        }
        catch (error) {
     ...
     */
    convertToSampleData(sourceFields) {
        const maxSamples = Math.max(...sourceFields.map((f) => f.sampleValues.length));
        const sampleData = [];
        for (let i = 0; i < Math.min(maxSamples, 3); i++) {
            const row = sourceFields.map((field) => field.sampleValues[i] !== undefined ? field.sampleValues[i] : null);
            sampleData.push(row);
        }
        return sampleData;
    }
...
//# sourceMappingURL=enhanced-import-service.js.map