...
export class ComplexFieldsStrategy extends BaseCSVStrategy {
    ...
    async execute(buffer, fileName) {
        ...
        try {
            ...
            const results = await this.tryComplexParsingStrategies(content, complexity);
            if (results.length === 0) {
                return this.createFailureResult("No successful complex parsing strategy", startTime);
            }
            // Select best result
            const bestResult = results.sort((a, b) => b.qualityScore - a.qualityScore)[0];
            const parseTime = Date.now() - startTime;
            const confidence = this.calculateConfidence(bestResult.data, complexity, bestResult.qualityScore, parseTime);
            return {
                success: true,
            ...
            return this.createFailureResult(error instanceof Error
                ? error.message
                : "Complex fields parsing failed", startTime);
        }
    }
    /**
    ...
    analyzeComplexity(content) {
        ...
        for (const line of lines) {
            if (line.includes('"')) {
                ...
                const quotedFields = fields.filter((f) => f.startsWith('"') && f.endsWith('"'));
     ...
     */
    async tryComplexParsingStrategies(content, complexity) {
        const results = [];
     ...
     */
    async parseRFC4180Relaxed(content, complexity) {
     ...
     */
    async parseCustomQuotes(content, complexity) {
        const records = parse(content, {
            columns: false,
            skip_empty_lines: true,
            trim: false,
     ...
     */
    async parseMultilineFields(content, complexity) {
     ...
     */
    async parseManualFieldExtraction(content, complexity) {
        const lines = content.split("\n");
        const records = [];
        for (const line of lines) {
            if (line.trim() === "")
     ...
     */
    extractComplexFields(line, complexity) {
        const fields = [];
        let currentField = "";
        let inQuotes = false;
        let i = 0;
     ...
     */
    processParseResults(records, complexity, strategy) {
        if (records.length === 0) {
            return { success: false };
        }
        // Clean up fields
        const cleanRecords = records.map((row) => row.map((cell) => this.cleanComplexCell(cell)));
        // Detect headers
        const hasHeaders = this.hasHeaders(cleanRecords.map((row) => row.join(complexity.delimiter)), complexity.delimiter);
        // Convert to object format
        let data;
        if (hasHeaders) {
            const headers = cleanRecords[0].map((header) => this.sanitizeFieldName(String(header)));
            data = cleanRecords.slice(1).map((row) => {
                const obj = {};
        ...
        const issues = this.identifyComplexIssues(data, cleanRecords, complexity, strategy);
        return {
            success: true,
            data,
            delimiter: complexity.delimiter,
     ...
     */
    parseLineBasic(line, delimiter) {
        return line.split(delimiter);
    }
    /**
     * Calculate confidence for complex fields parsing
     */
    calculateConfidence(data, complexity, qualityScore, parseTime) {
     ...
     */
    identifyComplexIssues(data, rawRecords, complexity, strategy) {
        const issues = [];
        ...
        if (complexity.hasMultilineFields && strategy !== "multiline") {
            issues.push("Multiline fields detected but not handled with multiline strategy");
        }
        ...
        if (uniqueLengths.size > 5) {
            issues.push("Highly variable field count - complex structure may not be fully resolved");
        }
...
//# sourceMappingURL=complex-fields-strategy.js.map