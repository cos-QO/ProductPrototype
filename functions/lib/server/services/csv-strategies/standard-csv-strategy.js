...
export class StandardCSVStrategy extends BaseCSVStrategy {
    ...
    async execute(buffer, fileName) {
        ...
        try {
            ...
            // Detect headers
            const hasHeaders = this.hasHeaders(records.map((row) => row.join(",")), ",");
            ...
            const confidence = this.calculateConfidence(data, qualityScore, parseTime);
            return {
                success: true,
                data,
                confidence,
            ...
            return this.createFailureResult(error instanceof Error ? error.message : "Standard CSV parsing failed", startTime);
        }
    }
    /**
     ...
     */
    calculateConfidence(data, qualityScore, parseTime) {
     ...
     */
    identifyIssues(data, rawRecords) {
        const issues = [];
        ...
        const hasQuotedFields = rawRecords.some((row) => row.some((cell) => typeof cell === "string" && cell.includes('"')));
        if (hasQuotedFields) {
        ...
        if (data.length > 0) {
            ...
            const emptyCells = data.reduce((count, row) => {
                return (count +
                    firstRowKeys.filter((key) => row[key] === null || row[key] === undefined || row[key] === "").length);
            }, 0);
            const emptyPercentage = (emptyCells / totalCells) * 100;
            if (emptyPercentage > 25) {
                issues.push(`High percentage of empty cells (${Math.round(emptyPercentage)}%)`);
            }
        }
        return issues;
    }
...
//# sourceMappingURL=standard-csv-strategy.js.map