...
function getTableName(table) {
    if (table === schema.users || table?.name === 'users')
        return 'users';
    if (table === schema.brands || table?.name === 'brands')
        return 'brands';
    if (table === schema.products || table?.name === 'products')
        return 'products';
    if (table === schema.mediaAssets || table?.name === 'media_assets')
        return 'mediaAssets';
    if (table === schema.productAttributes || table?.name === 'product_attributes')
        return 'productAttributes';
    if (table === schema.productVariations || table?.name === 'product_variations')
        return 'productVariations';
    if (table === schema.productVariationOptions || table?.name === 'product_variation_options')
        return 'productVariationOptions';
    if (table === schema.apiSyncs || table?.name === 'api_syncs')
        return 'apiSyncs';
    if (table === schema.productApiConfigs || table?.name === 'product_api_configs')
        return 'productApiConfigs';
    if (table === schema.syndicationLogs || table?.name === 'syndication_logs')
        return 'syndicationLogs';
    if (table === schema.channelProducts || table?.name === 'channel_products')
        return 'channelProducts';
    return null;
}
// Helper to extract value from Drizzle condition
function extractConditionValue(condition) {
    ...
    return {
        ...
        delete: (table) => ({
            where: async (condition) => {
                ...
                const value = extractConditionValue(condition);
                if (value !== null) {
                    const index = data.findIndex((item) => item.id === value || item.email === value);
                    if (index !== -1) {
                        data.splice(index, 1);
                    }
                }
