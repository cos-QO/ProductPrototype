    ...
    for (const row of data) {
        try {
            ...
            if (!row.name || !row.slug) {
                result.errors.push(`Row ${result.imported + result.failed + 1}: Missing required fields (name, slug)`);
                result.failed++;
                continue;
            }
            // Transform data
            ...
            result.errors.push(`Row ${result.imported + result.failed + 1}: ${error.message}`);
            result.failed++;
        }
    }
    ...
    for (const row of data) {
        try {
            ...
            if (row.price) {
                ...
                price = priceValue < 1000 && priceValue % 1 !== 0 ? Math.round(priceValue * 100) : Math.round(priceValue);
            }
            ...
            if (compareField) {
                ...
                // If price looks like dollars (has decimal), convert to cents
                compareAtPrice = compareValue < 1000 && compareValue % 1 !== 0 ? Math.round(compareValue * 100) : Math.round(compareValue);
            }
            const stock = row.stock ? parseInt(row.stock) : null;
            const weight = row.weight ? parseInt(row.weight) : null;
            const parentId = row.parent_id || row.parentId ? parseInt(row.parent_id || row.parentId) : null;
            ...
            const productData = {
                ...
                story: row.story || null,
                brandId: row.brand_id || row.brandId ? parseInt(row.brand_id || row.brandId) : null,
                parentId,
                sku: row.sku || null,
                gtin: row.gtin || null,
                ...
                compareAtPrice,
                stock,
                lowStockThreshold: row.low_stock_threshold || row.lowStockThreshold ?
                    parseInt(row.low_stock_threshold || row.lowStockThreshold) : null
            };
            // Insert into database
...
async function importProductAttributes(data) {
    const result = {
        success: false,
        imported: 0,
        failed: 0,
    ...
    for (const row of data) {
        try {
            ...
            if (!row.product_id || !row.attribute_name) {
                result.errors.push(`Row ${result.imported + result.failed + 1}: Missing required fields`);
                result.failed++;
                continue;
            }
            const attributeData = {
            ...
            const [inserted] = await db.insert(productAttributes)
        ...
        products: [
            {
                ...
                short_description: 'Noise-canceling headphones with 30-hour battery life',
                long_description: 'Professional-grade wireless headphones featuring active noise cancellation, premium drivers, and extended battery life. Perfect for music production, gaming, and daily listening.',
                story: 'Engineered by audio professionals who understand that great sound shouldn\'t compromise on comfort or convenience.',
                brand_id: 1,
                parent_id: null,
                sku: 'AT-WH-001',
                gtin: '1234567890123',
            ...
            },
            {
                ...
                short_description: 'Sustainable organic cotton t-shirt in multiple colors',
                long_description: 'Made from 100% certified organic cotton, this comfortable t-shirt features a classic fit and is available in a range of colors. Ethically sourced and environmentally friendly.',
                story: 'Part of our commitment to sustainable fashion, supporting organic farming communities worldwide.',
                brand_id: 2,
                parent_id: null,
                sku: 'OC-TS-001',
                gtin: '2345678901234',
            ...
            },
            {
                ...
                long_description: 'Handcrafted coffee table made from sustainably sourced bamboo. Features a minimalist design with clean lines and a natural finish that complements any living space.',
                story: 'Each piece is carefully crafted by skilled artisans who take pride in creating furniture that\'s both beautiful and environmentally responsible.',
                brand_id: 3,
                parent_id: null,
                sku: 'BF-CT-001',
                gtin: '3456789012345',
