...
export const secureFileUpload = (options = {}) => {
    const { allowedTypes = ['documents'], maxFileSize = 50 * 1024 * 1024, // 50MB default
    maxFiles = 1, destination = 'uploads/' } = options;
...
function getExpectedMimeType(ext) {
    const mimeMap = {
        '.jpg': ['image/jpeg'],
        '.jpeg': ['image/jpeg'],
        '.png': ['image/png'],
...
const csrfTokens = global.csrfTokens;
...
export const secureFileServing = (baseDir) => {
    return (req, res, next) => {
