import { db } from './db';
import { fieldMappingCache, type FieldMappingCache, type InsertFieldMappingCache } from '@shared/schema';
...
interface LLMFieldMappingRequest {
  headers: string[];
  sampleData: any[][];
  targetFields: string[];
}

interface LLMFieldMappingResponse {
  mappings: Array<{
    sourceField: string;
    targetField: string;
    confidence: number;
...
export class FieldMappingEngine {
  private static instance: FieldMappingEngine;
  private openRouterClient: OpenRouterClient;
  private targetFields: string[] = Object.keys(PRODUCT_FIELD_DEFINITIONS);

  static getInstance(): FieldMappingEngine {
   ...
   */
  async generateMappings(sourceFields: SourceField[]): Promise<FieldMapping[]> {
    const mappings: FieldMapping[] = [];
    const unmappedFields: SourceField[] = [];
    ...
    for (const mapping of historicalMappings) {
      if (mapping.confidence > 60) {
        ...
        if (index > -1) unmappedFields.splice(index, 1);
      }
    }
   ...
   * Strategy 1: Exact Match
   */
  private getExactMatch(sourceField: SourceField): FieldMapping | null {
   ...
   * Strategy 2: Fuzzy Match
   */
  private getFuzzyMatch(sourceField: SourceField): FieldMapping | null {
    const sourceName = sourceField.name.toLowerCase().replace(/[_\s-]/g, '');
    let bestMatch: { target: string, confidence: number } = { target: '', confidence: 0 };
   ...
   * Strategy 3: Historical Learning
   */
  private async getHistoricalMappings(sourceFields: SourceField[]): Promise<FieldMapping[]> {
    const mappings: FieldMapping[] = [];
   ...
   * Strategy 4: Statistical Inference
   */
  private getStatisticalMapping(sourceField: SourceField): FieldMapping | null {
   ...
   */
  private async getLLMMappings(sourceFields: SourceField[]): Promise<FieldMapping[]> {
    ...
    try {
      ...
      const result = await this.openRouterClient.analyzeFieldMapping(
        headers,
        transposedData,
        this.targetFields,
        'Product data import field mapping'
      );
  ...
  private convertLLMResponseToMappings(
    llmMappings: Array<{
      sourceField: string;
      targetField: string;
      confidence: number;
  ...
  private async cacheMappings(mappings: FieldMapping[], sourceFields: SourceField[]) {
    try {
      for (const mapping of mappings) {
        ...
        const existing = await db.select()
          ...
          .where(
            and(
              eq(fieldMappingCache.sourceField, mapping.sourceField),
              eq(fieldMappingCache.targetField, mapping.targetField)
            )
          )
          .limit(1);
   ...
   */
  async getSuggestionsForField(sourceField: SourceField): Promise<FieldMapping[]> {
    ...
    return suggestions
      ...
      .filter((mapping, index, self) => 
        self.findIndex(m => m.targetField === mapping.targetField) === index
      );
  }
   ...
   */
  validateMappings(mappings: FieldMapping[]): { isValid: boolean, errors: string[], warnings: string[] } {
    const errors: string[] = [];
    const warnings: string[] = [];
    const targetFieldsUsed = new Set<string>();
