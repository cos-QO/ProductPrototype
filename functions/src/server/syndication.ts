...
export class SyndicationService {
  private async logSyndication(
    ...
    const log: InsertSyndicationLog = {
      channelId,
      productId,
      action,
      endpoint,
  ...
  private async makeQueenOneApiCall(
    endpoint: string,
    method: string,
    payload: any,
    apiKey: string
  ...
  private buildProductPayload(product: Product, channel: SyndicationChannel): any {
    const basePayload = {
      name: product.name,
      slug: product.slug,
  ...
  async syndicateProduct(
    ...
    try {
      ...
      const errorResult: SyndicationResult = {
        success: false,
        responseTime: 0,
        status: 500,
        error: error instanceof Error ? error.message : 'Unknown error',
  ...
  async syndicateToAllChannels(
    ...
    const promises = channels.map(channel =>
      this.syndicateProduct(product, channel.id!, action, triggeredBy)
    );
  ...
  async getSyndicationStatus(productId: number): Promise<{
    total: number;
    live: number;
    pending: number;
    error: number;
