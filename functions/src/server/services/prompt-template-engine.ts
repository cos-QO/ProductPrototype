...
/**
 * Advanced Prompt Template Engine with Variable Injection System
 * 
 * Key Features:
 * - [UPLOADED_FIELDS] variable substitution
 * - External file-based prompt management
 * - Hot-reload capability for development
 * - Template compilation with complex data structures
 * - A/B testing support
...
interface DynamicPromptEngine {
  compile(templateId: string, variables: Record<string, any>): Promise<string>;
  addVariable(name: string, value: any, formatter?: (value: any) => string): void;
  render(template: string, context: CompilationContext): string;
  loadTemplate(templateId: string): Promise<PromptTemplate>;
  reloadTemplates(): Promise<void>;
}

export class PromptTemplateEngine implements DynamicPromptEngine {
  private static instance: PromptTemplateEngine;
  private templates: Map<string, PromptTemplate> = new Map();
  private watchers: Map<string, fs.FSWatcher> = new Map();
  private templatesPath: string;
  private globalVariables: Map<string, TemplateVariable> = new Map();
  private isHotReloadEnabled: boolean = process.env.NODE_ENV === 'development';
   ...
   */
  async compile(templateId: string, variables: Record<string, any> = {}): Promise<string> {
    try {
   ...
   */
  private formatUploadedFields(uploadedFields: any): string {
    ...
    if (uploadedFields.fields && uploadedFields.sampleData) {
      ...
      uploadedFields.fields.forEach((field: any, index: number) => {
      ...
      if (uploadedFields.sampleData && uploadedFields.sampleData.length > 0) {
        ...
        uploadedFields.sampleData.slice(0, 3).forEach((row: any[], rowIndex: number) => {
   ...
   */
  private processConditionalBlocks(template: string, context: CompilationContext): string {
   ...
   */
  private processLoopBlocks(template: string, context: CompilationContext): string {
   ...
   */
  private getDefaultFormatters(): Record<string, (value: any) => string> {
    return {
   ...
   */
  private formatSkuFields(skuFields: Record<string, string>): string {
   ...
   */
  private formatFieldMappings(mappings: any[]): string {
    if (!Array.isArray(mappings)) return '';
    
    return mappings.map(mapping => 
      `${mapping.sourceField} → ${mapping.targetField} (${mapping.confidence}%)`
    ).join('\n');
  }
   ...
   */
  private defaultVariableFormatter(value: any): string {
   ...
   */
  private async initializeTemplatesDirectory(): Promise<void> {
    try {
  ...
  private setupHotReload(): void {
    if (this.watchers.size > 0) return; // Already setup
  ...
  private async createDefaultTemplates(): Promise<void> {
    const fieldMappingTemplate: PromptTemplate = {
      id: 'field-mapping-v1',
      version: '1.0.0',
...
RESPONSE FORMAT (JSON only):
{
   ...
   */
  getTemplateMetadata(templateId: string): PromptTemplate['metadata'] | null {
    const template = this.templates.get(templateId);
