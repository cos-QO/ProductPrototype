...
export class DirtyCSVRecoveryStrategy extends BaseCSVStrategy {
    ...
    try {
      ...
      if (!recoveryResults.success) {
        return this.createFailureResult(
          "Unable to recover any data from file",
          startTime,
        );
      }
      ...
      const confidence = this.calculateConfidence(
        recoveryResults.data,
        damage,
        recoveryResults.recoveryScore,
        parseTime,
      );
      ...
      return this.createFailureResult(
        error instanceof Error ? error.message : "Dirty CSV recovery failed",
        startTime,
      );
    }
  }
   ...
   */
  private analyzeDamage(content: string): any {
    const damage = {
      encoding: "utf8",
      hasNullBytes: false,
      hasBinaryData: false,
    ...
    const linesWithoutDelimiters = lines.filter(
      (line) => !line.includes(damage.estimatedDelimiter) && line.trim() !== "",
    ).length;
    ...
    const linesWithTrailing = lines.filter(
   ...
   */
  private detectDelimiterFromMess(content: string): string {
    const delimiters = [",", ";", "\t", "|", ":", " "];
    ...
    for (const delimiter of delimiters) {
      ...
      const counts = lines.map(
        (line) =>
          (
            line.match(
              new RegExp(delimiter === " " ? "\\s+" : `\\${delimiter}`, "g"),
            ) || []
          ).length,
      );
   ...
   */
  private async attemptRecovery(content: string, damage: any): Promise<any> {
    const recoverySteps = [
   ...
   */
  private async basicCleanupRecovery(
    content: string,
    damage: any,
  ): Promise<any> {
    let cleaned = content;
    ...
    return this.parseCleanedContent(
      cleaned,
      damage.estimatedDelimiter,
      "basic-cleanup",
    );
  }
   ...
   */
  private async aggressiveCleanupRecovery(
    content: string,
    damage: any,
  ): Promise<any> {
    let cleaned = content;
   ...
   */
  private async lineByLineRecovery(content: string, damage: any): Promise<any> {
    const lines = content.split(/\r?\n/);
   ...
   */
  private async desperateRecovery(content: string, damage: any): Promise<any> {
   ...
   */
  private recoverLine(line: string, delimiter: string): string | null {
   ...
   */
  private estimateFieldsPerRow(fields: string[]): number {
    ...
    for (const size of possibleRowSizes) {
      if (fields.length < size * 2) continue; // Need at least 2 complete rows
      ...
      for (let col = 0; col < size; col++) {
        ...
        else if (uniqueTypes.size <= 2) score += 1; // Mostly same type
      }
   ...
   */
  private async parseCleanedContent(
    content: string,
    delimiter: string,
    method: string,
  ): Promise<any> {
   ...
   */
  private parseLineManually(line: string, delimiter: string): string[] {
    const fields: string[] = [];
    let currentField = "";
    let inQuotes = false;
  ...
  private createRecoveryResult(
    ...
    const data = rows.slice(startRow).map((row: string[]) => {
      const obj: any = {};
   ...
   */
  private cleanRecoveredValue(value: string): any {
  ...
  private calculateRecoveryScore(
    ...
    // Method penalties
    const methodPenalties: Record<string, number> = {
      "basic-cleanup": 0,
    ...
    if (data.length > 0) {
      ...
      const structureConsistency =
        data.filter((row) => Object.keys(row).length === firstRowKeys.length)
          .length / data.length;
   ...
   */
  private identifyRecoveryIssues(
    data: any[],
    method: string,
    damage: any,
  ): string[] {
    ...
    switch (method) {
      case "aggressive-cleanup":
        issues.push(
          "Aggressive data cleaning applied - some information may be lost",
        );
        break;
      case "line-by-line":
        issues.push(
          "Line-by-line recovery used - data structure may be altered",
        );
        break;
      case "desperate":
        issues.push(
          "Desperate recovery mode - data accuracy cannot be guaranteed",
        );
        break;
    }
    ...
    if (data.length > 0) {
      ...
      if (nullPercentage > 50) {
        issues.push(
          `High percentage of missing data (${Math.round(nullPercentage)}%)`,
        );
      }
    }
  ...
  private calculateConfidence(
    ...
    if (parseTime < 1000) confidence += 2;
