...
interface WorkflowTransition {
  from: WorkflowState;
  to: WorkflowState;
  trigger: "automatic" | "user_action" | "confidence_threshold";
...
export class WorkflowOrchestrator {
  ...
  private enhancedImportService: EnhancedImportService;
  private confidenceThreshold = 0.7; // 70% confidence for auto-advancement
  ...
  private transitions: WorkflowTransition[] = [
    ...
    },
    {
      ...
      condition: async (context) =>
        (context.confidence || 0) >= this.confidenceThreshold,
   ...
   */
  initialize(enhancedImportService: EnhancedImportService): void {
   ...
   */
  private getEnhancedImportService(): EnhancedImportService {
   ...
   */
  async handleAnalysisComplete(context: WorkflowContext): Promise<void> {
    try {
      ...
      console.log(
        `[WORKFLOW] Analysis complete for session ${sessionId}, confidence: ${averageConfidence}%`,
      );
      ...
      if (averageConfidence >= this.confidenceThreshold) {
        console.log(
          `[WORKFLOW] Auto-advancing to preview generation (confidence: ${averageConfidence}%)`,
        );
        ...
        console.log(
          `[WORKFLOW] Requesting user mapping intervention (confidence: ${averageConfidence}%)`,
        );
   ...
   */
  async autoGeneratePreview(context: WorkflowContext): Promise<void> {
    try {
      const { sessionId } = context;

      console.log(
        `[WORKFLOW] Starting auto preview generation for session ${sessionId}`,
      );

      // Update workflow state
      await this.updateWorkflowState(
        sessionId,
        WorkflowState.GENERATING_PREVIEW,
      );
      ...
      const previewResult =
        await this.getEnhancedImportService().generatePreviewInternal(
          sessionId,
        );

      if (!previewResult.success) {
        throw new Error(
          previewResult.errors?.join(", ") || "Preview generation failed",
        );
      }
   ...
   */
  async handlePreviewGenerated(context: WorkflowContext): Promise<void> {
    try {
      const { sessionId } = context;

      console.log(
        `[WORKFLOW] Preview generated successfully for session ${sessionId}`,
      );
   ...
   */
  async advanceToApproval(context: WorkflowContext): Promise<void> {
    try {
      const { sessionId } = context;

      console.log(
        `[WORKFLOW] Advancing to approval step for session ${sessionId}`,
      );
  ...
  async emitWorkflowEvent(
    sessionId: string,
    event: Omit<WorkflowEvent, "sessionId" | "timestamp">,
  ): Promise<void> {
    try {
      ...
      console.log(
        `[WORKFLOW] Event emitted: ${event.type} for session ${sessionId}`,
      );
    } catch (error) {
  ...
  async handleWorkflowError(
    ...
    try {
      console.error(
        `[WORKFLOW] Error in ${context} for session ${sessionId}:`,
        error,
      );
      ...
      console.error(
        "[WORKFLOW] Critical error in error handling:",
        fallbackError,
      );
    }
  }
   ...
   */
  private async implementFallbackStrategy(
    sessionId: string,
    errorContext: string,
    error: any,
  ): Promise<void> {
    switch (errorContext) {
      case "preview_generation":
        ...
        await this.emitWorkflowEvent(sessionId, {
          ...
          payload: {
            message:
              "Automatic preview generation failed. Please generate preview manually.",
      ...
      case "analysis_complete":
        ...
        await this.emitWorkflowEvent(sessionId, {
          ...
          payload: {
            message:
              "Automatic field mapping failed. Please review mappings manually.",
            fallbackAction: "manual_mapping_required",
            instruction:
              "Review and adjust field mappings in the processing step",
          },
        });
        break;
      ...
      default:
        ...
        await this.emitWorkflowEvent(sessionId, {
          ...
          payload: {
            message:
              "Workflow automation encountered an issue. Manual controls are now enabled.",
  ...
  async executeWorkflow(
    ...
    try {
      ...
      for (const transition of applicableTransitions) {
        ...
        if (canTransition) {
          console.log(
            `[WORKFLOW] Executing transition: ${transition.from} -> ${transition.to}`,
          );
  ...
  async getWorkflowStatus(sessionId: string): Promise<{
    ...
    try {
      ...
      const currentState =
        (session.workflowState as WorkflowState) || WorkflowState.INITIATED;
   ...
   */
  private getNextAction(state: WorkflowState, canAutoAdvance: boolean): string {
    switch (state) {
