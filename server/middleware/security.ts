...
export const secureFileUpload = (
  ...
  const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      ...
      const safeName =
        file.fieldname + "-" + timestamp + "-" + randomSuffix + ext;
      cb(null, safeName);
    },
  });
    ...
    try {
      ...
      if (!allowedMimeTypes.includes(file.mimetype)) {
        ...
        if (file.mimetype === "application/octet-stream") {
          ...
          if (!documentExtensions.includes(ext)) {
            return cb(
              new Error(
                `File type ${ext} with MIME type ${file.mimetype} is not allowed`,
              ),
            );
          }
      ...
      if (file.mimetype !== "application/octet-stream") {
        ...
        if (expectedMimeForExt && !expectedMimeForExt.includes(file.mimetype)) {
          return cb(
            new Error(
              `MIME type ${file.mimetype} does not match file extension ${ext}`,
            ),
          );
        }
      }
...
export const scanFileContent = async (
  ...
  try {
    ...
    if (content.length > stats.size * 10) {
      // Content 10x larger than file
...
if (!(global as any).csrfTokens) {
  (global as any).csrfTokens = new Map<
    string,
...
export const csrfProtection = (
  ...
  if (process.env.NODE_ENV === "development" || !process.env.NODE_ENV) {
    console.log(
      `[CSRF] Skipping CSRF protection in development for ${req.method} ${req.path}`,
    );
    return next();
  }
...
export const validateFilePath = (filePath: string): boolean => {
  ...
  // Remove leading slash for validation (Express req.path includes it)
