...
export class CSVConfidenceScorer {
  private static instance: CSVConfidenceScorer;

  static getInstance(): CSVConfidenceScorer {
   ...
   */
  private calculateStructuralConsistency(data: any[]): number {
    ...
    const consistentRows = data.filter(
      (row) => Object.keys(row).length === expectedColumns,
    ).length;
    ...
    const keyConsistency =
      (data.slice(1).reduce((acc, row) => {
        ...
        const matchingKeys = firstRowKeys.filter((key) =>
   ...
   */
  private calculateTypeConsistency(data: any[]): number {
    if (data.length === 0) return 0;

    const firstRowKeys = Object.keys(data[0] || {});

    if (firstRowKeys.length === 0) return 0;
   ...
   */
  private calculateDelimiterReliability(metadata: any): number {
    if (!metadata.delimiter) return 50; // Default if no delimiter info
   ...
   */
  private calculateHeaderQuality(data: any[], metadata: any): number {
    if (data.length === 0) return 0;
    ...
    if (hasHeaders) {
      ...
      const genericHeaders = firstRowKeys.filter((key) =>
        /^(column_\d+|field_\d+|col\d+)$/i.test(key),
      ).length;
      const nonGenericRatio =
        (firstRowKeys.length - genericHeaders) / firstRowKeys.length;
      score += nonGenericRatio * 40;

      // Descriptive headers
      const descriptiveHeaders = firstRowKeys.filter(
        (key) => key.length > 3 && /[a-zA-Z]/.test(key),
      ).length;
   ...
   */
  private calculateDataVariety(data: any[]): number {
    if (data.length === 0) return 0;
   ...
   */
  private calculateParseEfficiency(
    parseTime: number,
    recordCount: number,
  ): number {
    ...
    if (timePerRecord < 1) return 95;
    if (timePerRecord < 5) return 85;
    if (timePerRecord < 20) return 70;
    if (timePerRecord < 50) return 50;
    return 30;
  }
   ...
   */
  private calculateErrorRate(metadata: any): number {
    if (!metadata.issues || !Array.isArray(metadata.issues)) return 100;
   ...
   */
  private aggregateScore(metrics: ConfidenceMetrics): number {
    const weights = {
    ...
    const weightedScore = Object.entries(weights).reduce(
      (score, [metric, weight]) => {
        return score + metrics[metric as keyof ConfidenceMetrics] * weight;
      },
      0,
    );
   ...
   */
  private identifyPositiveFactors(metrics: ConfidenceMetrics): string[] {
    const factors: string[] = [];

    if (metrics.structuralConsistency > 90)
      factors.push("Excellent structural consistency");
    if (metrics.dataCompleteness > 85) factors.push("High data completeness");
    if (metrics.typeConsistency > 80) factors.push("Strong type consistency");
    if (metrics.delimiterReliability > 85)
      factors.push("Reliable delimiter detection");
    if (metrics.headerQuality > 75) factors.push("Quality column headers");
    if (metrics.dataVariety > 60) factors.push("Good data variety");
    if (metrics.parseEfficiency > 80) factors.push("Fast parsing performance");
    if (metrics.errorRate > 95) factors.push("Error-free parsing");
   ...
   */
  private identifyIssues(metrics: ConfidenceMetrics): string[] {
    const issues: string[] = [];
    ...
    if (metrics.dataVariety < 30) issues.push("Limited data variety detected");
    if (metrics.parseEfficiency < 50) issues.push("Slow parsing performance");
    if (metrics.errorRate < 80) issues.push("Multiple parsing errors detected");
  ...
  private generateRecommendations(
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        "Data quality is good - no specific recommendations",
      );
    }
   ...
   */
  private createFailureResult(reason: string): ConfidenceResult {
    return {
      score: 0,
      metrics: {
