...
export class FieldExtractionService {
  ...
  async extractFieldsFromFile(
    ...
    try {
      ...
      throw new Error(
        `Field extraction failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  private parseCSVData(buffer: Buffer): {
    data: any[];
    hasHeaders: boolean;
    totalRecords: number;
  } {
   ...
   */
  private parseJSONData(buffer: Buffer): {
    data: any[];
    hasHeaders: boolean;
    totalRecords: number;
  } {
   ...
   * Parse XLSX data
   */
  private parseXLSXData(buffer: Buffer): {
    data: any[];
    hasHeaders: boolean;
    totalRecords: number;
  } {
  ...
  private async analyzeField(
    ...
    if (options.inferTypes) {
      metadata.inferredType = this.advancedTypeInference(
        nonNullValues,
        fieldName,
      );
    }
  ...
  private calculateExtractionConfidence(
    ...
    const hasGoodFieldNames =
      fields.filter(
        (f) => f.name && f.name.length > 1 && !f.name.startsWith("column_"),
    ...
    const avgNullPercentage =
      fields.reduce((sum, f) => sum + f.nullPercentage, 0) / fields.length;
