...
interface SimplifiedMappingResult {
  success: boolean;
...
export class SimpleFieldMappingService {
  private static instance: SimpleFieldMappingService;
  private openRouterClient: OpenRouterClient;

  static getInstance(): SimpleFieldMappingService {
   ...
   */
  async processFileForMapping(fileData: ExtractedFileFields): Promise<SimplifiedMappingResult> {
   ...
   */
  extractFieldsFromFile(fileContent: any, fileType: 'csv' | 'json' | 'xlsx'): ExtractedFileFields {
    let fields: string[] = [];
    let sampleData: any[][] = [];
   ...
   */
  private buildCentralizedSystemPrompt(): string {
...
IGNORE UNMATCHED: If a field doesn't match well, simply don't include it in mappings.`;
   ...
   */
  private buildUserPrompt(fileData: ExtractedFileFields): string {
   ...
   */
  private getUnmappedFields(sourceFields: string[], mappings: FieldMapping[]): string[] {
  ...
  private extractFieldsFromJSON(jsonContent: any): string[] {
  ...
  private extractSampleDataFromJSON(jsonContent: any, fields: string[]): any[][] {
    if (Array.isArray(jsonContent)) {
      return jsonContent.slice(0, 3).map(item => 
  ...
  private extractFieldsFromCSV(csvContent: string): string[] {
  ...
  private extractSampleDataFromCSV(csvContent: string, fields: string[]): any[][] {
    const lines = csvContent.trim().split('\n');
  ...
  private extractFieldsFromXLSX(xlsxContent: any): string[] {
  ...
  private extractSampleDataFromXLSX(xlsxContent: any, fields: string[]): any[][] {
