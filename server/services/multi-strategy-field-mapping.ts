...
import {
  ...
  type SourceField,
  type ExtractedFileFields,
} from "./field-extraction-service.js";
import { db } from "../db";
...
export class MultiStrategyFieldMapping {
  ...
  private readonly MAX_COST_PER_SESSION = 0.005; // $0.005 - increased for realistic LLM usage
  ...
  async generateMappings(
    ...
    try {
      ...
      const finalMappings = await this.aggregateResults(
        strategyResults,
        context,
      );
  ...
  private async strategy1ExactMatching(context: any): Promise<StrategyResult> {
    ...
    for (const sourceField of context.sourceFields) {
      ...
      const targetField = Object.keys(SKU_FIELDS).find(
        (target) => target.toLowerCase() === fieldName,
      );
      ...
      if (bestMatch && bestMatch.score > 0.7) {
        ...
        mappings.push({
          ...
          metadata: {
            ...
            dataTypeMatch: this.checkDataTypeMatch(
              sourceField,
              bestMatch.target,
            ),
          },
   ...
   */
  private async strategy3SemanticAnalysis(
    context: any,
  ): Promise<StrategyResult> {
    const startTime = Date.now();
    const mappings: FieldMapping[] = [];
   ...
   */
  private async strategy4HistoricalLearning(
    context: any,
  ): Promise<StrategyResult> {
    const startTime = Date.now();
    const mappings: FieldMapping[] = [];
    ...
    try {
      ...
      for (const sourceField of context.sourceFields) {
        ...
        for (const historical of historicalMappings) {
          const similarity = this.calculateStringSimilarity(
            fieldName,
            historical.sourceField.toLowerCase(),
          );
  ...
  private async strategy5LLMPowered(context: any): Promise<StrategyResult> {
    ...
    if (
      !this.openRouterClient.isAvailable() ||
    ...
    try {
      ...
      const prompt = await this.promptEngine.compile(
        "field-mapping-v1",
        templateVariables,
      );
      ...
      // Parse LLM response
      const llmMappings = this.parseLLMResponse(
      ...
      return {
        ...
        confidence:
          llmMappings.length > 0
            ? Math.round(
                llmMappings.reduce((sum, m) => sum + m.confidence, 0) /
                  llmMappings.length,
              )
            : 0,
        processingTime: Date.now() - startTime,
  ...
  private async aggregateResults(
    ...
    const overallConfidence =
      finalMappings.length > 0
        ? Math.round(
            finalMappings.reduce((sum, m) => sum + m.confidence, 0) /
    ...
    return [
      {
        ...
        contentMatcher: (values: any[]) =>
          values.some((v) => typeof v === "string" && v.length > 3),
      },
      {
        ...
        contentMatcher: (values: any[]) =>
          values.some((v) => /^[A-Z0-9\-_]+$/i.test(String(v))),
      },
      {
        ...
        contentMatcher: (values: any[]) =>
          values.some((v) => !isNaN(parseFloat(String(v)))),
      },
      {
        ...
        contentMatcher: (values: any[]) =>
          values.some((v) => typeof v === "string" && v.length > 10),
      },
      {
        ...
        contentMatcher: (values: any[]) =>
          values.some((v) => Number.isInteger(Number(v))),
      },
      {
        ...
        contentMatcher: (values: any[]) =>
          values.some((v) => /^\d{8,14}$/.test(String(v))),
      },
    ];
  }
   ...
   */
  private async getHistoricalMappingsForContext(): Promise<string | null> {
    try {
      ...
      return recentMappings
        .map(
          (mapping) =>
            `${mapping.sourceField} → ${mapping.targetField} (${mapping.confidence}% via ${mapping.strategy}, used ${mapping.usageCount} times)`,
        )
        .join("\n");
    } catch (error) {
