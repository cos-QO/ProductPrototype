...
export class NumericHeaderlessStrategy extends BaseCSVStrategy {
    ...
    try {
      ...
      return this.createFailureResult(
        error instanceof Error
          ? error.message
          : "Numeric headerless parsing failed",
        startTime,
      );
    }
  }
   ...
   * Detect the best delimiter for numeric data
   */
  private detectBestDelimiter(content: string): string {
    const delimiters = [",", ";", "\t", "|"];
   ...
   */
  private validateNumericHeaderless(records: any[][]): {
    isValid: boolean;
    reason: string;
    numericRatio: number;
  } {
   ...
   */
  private generateSmartColumnNames(records: any[][]): string[] {
   ...
   */
  private analyzeColumnType(columnData: any[]): string {
    if (columnData.length === 0) return "column";
    ...
    if (integerCount / total > 0.8) return "integer";
    if (decimalCount / total > 0.8) return "decimal";
    if ((integerCount + decimalCount) / total > 0.8) return "numeric";
   ...
   */
  private cleanNumericValue(value: any): any {
  ...
  private identifyIssues(
    ...
    if (numericRatio < 0.8) {
      issues.push(
        `Mixed data types detected (${Math.round(numericRatio * 100)}% numeric)`,
      );
    }

    // Check for potential precision issues
    const hasHighPrecisionDecimals = rawRecords.some((row) =>
      row.some((cell) => {
    ...
    if (hasHighPrecisionDecimals) {
      issues.push(
        "High precision decimal values detected - verify numeric accuracy",
      );
    }

    // Check for scientific notation
    const hasScientificNotation = rawRecords.some((row) =>
      row.some((cell) => {
        const str = String(cell);
    ...
    if (data.length > 0) {
      ...
      const nullPercentage =
        (nullCells / (data.length * firstRowKeys.length)) * 100;
      if (nullPercentage > 15) {
        issues.push(
          `High percentage of missing values (${Math.round(nullPercentage)}%)`,
        );
      }
    }
