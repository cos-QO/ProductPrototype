...
import { CSVStrategy, CSVParseResult } from "../adaptive-csv-extractor";

export abstract class BaseCSVStrategy implements CSVStrategy {
   ...
   */
  abstract execute(buffer: Buffer, fileName: string): Promise<CSVParseResult>;

  /**
   * Check if strategy can handle the given CSV data
   */
  abstract canHandle(buffer: Buffer): boolean;
  ...
  getConfidenceBonus(data: any[]): number {
    ...
    const consistentColumns = data.every(
      (row) => Object.keys(row).length === firstRowKeys.length,
    );
    if (consistentColumns) bonus += 20;
    ...
    if (emptyPercentage < 10) bonus += 15;
    else if (emptyPercentage < 25) bonus += 10;
    else if (emptyPercentage < 50) bonus += 5;
  ...
  protected hasHeaders(lines: string[], delimiter: string): boolean {
    ...
    const firstRowAllText = firstRow.every(
      (cell) => isNaN(Number(cell.trim())) && cell.trim() !== "",
    );
  ...
  protected cleanCellValue(value: string): any {
  ...
  protected calculateDataQuality(data: any[]): number {
  ...
  protected generateColumnNames(columnCount: number): string[] {
