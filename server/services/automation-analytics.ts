...
// Temporarily removed database imports to prevent SQL errors
// import { db } from "../db";
// import {
//   generatedTestCases,
//   testExecutions,
...
export class AutomationAnalyticsService {
  ...
  async getAutomationMetrics(
    ...
    try {
      // For now, return mock data to prevent SQL errors while database is being set up
      // TODO: Remove this fallback once database has test data
  ...
  private async calculateAutomationRate(timeWindow: Date) {
    try {
      ...
      const previousPeriodData = await this.getPreviousPeriodAutomationRate(
        timeWindow,
      ).catch(() => 0);

      const current = totalTests[0]?.count
        ? (automatedTests[0]?.count / totalTests[0].count) * 100
        : 0;
      ...
      console.error("Error calculating automation rate:", error);
    ...
    try {
      ...
      const productivity =
        estimatedManualTime > 0
          ? (savedHours / (estimatedManualTime / 60)) * 100
          : 0;
  ...
  private async calculateErrorReduction(timeWindow: Date) {
    try {
      ...
      const automatedTests = testResults.filter(
        (t) => t.automationLevel === "full",
      );
      const manualTests = testResults.filter(
        (t) => t.automationLevel === "manual",
      );

      const automationAccuracy =
        automatedTests.length > 0
          ? (automatedTests.filter((t) => t.status === "passed").length /
      ...
      const manualAccuracy =
        manualTests.length > 0
          ? (manualTests.filter((t) => t.status === "passed").length /
              manualTests.length) *
            100
      ...
      console.error("Error calculating error reduction:", error);
      return {
  ...
  private async calculateUserSatisfaction(timeWindow: Date) {
    ...
    const approvalData = await db
      ...
      .where(
        and(
          gte(testExecutions.createdAt, timeWindow),
          or(
            eq(testExecutions.status, "passed"),
            eq(testExecutions.status, "failed"),
          ),
        ),
      );
    ...
    const successRate =
      approvalData.filter((t) => t.status === "passed").length /
      approvalData.length;
    const feedbackScore = Math.min(
      100,
      successRate * 100 - avgApprovalTime * 2,
    );
  ...
  private async calculateSystemHealth(timeWindow: Date) {
    ...
    const performance =
      systemMetrics.avgResponseTime < 1000
        ? 95
        : systemMetrics.avgResponseTime < 2000
          ? 85
          : 75;
    const resourceUsage = systemMetrics.resourceUsage || 60;

    let status: "healthy" | "warning" | "critical" = "healthy";
    if (uptime < 99 || performance < 80 || resourceUsage > 90)
      status = "critical";
    else if (uptime < 99.5 || performance < 90 || resourceUsage > 80)
      status = "warning";
  ...
  async getBusinessImpact(
    ...
    const timeToMarket =
      await this.calculateTimeToMarketImprovement(timeWindow);

    // Calculate quality improvement metrics
    const qualityImprovement =
      await this.calculateQualityImprovement(timeWindow);
  ...
  async getOptimizationRecommendations(): Promise<OptimizationRecommendations> {
    ...
    const performanceOptimization =
      await this.generatePerformanceOptimizations(metrics);
    const automationOpportunities =
      await this.identifyAutomationOpportunities(metrics);
  ...
  private async getPreviousPeriodAutomationRate(
    currentWindow: Date,
  ): Promise<number> {
    try {
      const previousWindow = new Date(
        currentWindow.getTime() - (Date.now() - currentWindow.getTime()),
      );
      ...
      return totalTests[0]?.count
        ? (automatedTests[0]?.count / totalTests[0].count) * 100
        : 0;
    } catch (error) {
      console.error(
        "Error calculating previous period automation rate:",
        error,
      );
      return 0;
    }
  ...
  private async getManualProcessingCosts(timeWindow: Date): Promise<number> {
    try {
      ...
      console.error("Error calculating manual processing costs:", error);
      return 0;
    }
  }
  ...
  private async identifyBottlenecks(timeWindow: Date): Promise<string[]> {
    ...
    const avgTimes = testResults.reduce(
      (acc, test) => {
    ...
    const failureRates = testResults.reduce(
      (acc, test) => {
  ...
  private async getDayMetrics(date: Date) {
    ...
    const automated = dayTests.filter(
      (t) => t.automationLevel === "full",
    ).length;
  ...
  private async calculateTimeToMarketImprovement(
    timeWindow: Date,
  ): Promise<number> {
  ...
  private async generateCostOptimizations(metrics: AutomationMetrics) {
    ...
    if (metrics.costEfficiency.llmCosts > 100) {
      recommendations.push({
        recommendation:
          "Optimize LLM token usage by implementing response caching",
