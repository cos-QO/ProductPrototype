import { db } from '../db';
import { fieldMappingCache, eq, desc, and, sql, gt } from '@shared/schema';
...
export class LearningSystem {
        ...
        await db.update(fieldMappingCache)
          .set({
            ...
            metadata: {
              ...current.metadata,
              ...metadata,
              strategies: [...(current.metadata.strategies || []), strategy].slice(-10) // Keep last 10 strategies
            }
          })
          .where(eq(fieldMappingCache.sourceFieldPattern, normalizedSource));
   ...
   */
  private async getSuggestionsForField(sourceField: string): Promise<LearningSuggestion> {
   ...
   */
  private async getFuzzyMatches(normalizedField: string): Promise<LearningInsight[]> {
    ...
    return fuzzyMatches.sort((a, b) => b.predictedConfidence - a.predictedConfidence).slice(0, 2);
  }

  /**
   * Get partial pattern matches (for compound field names)
   */
  private async getPartialMatches(normalizedField: string): Promise<LearningInsight[]> {
    ...
    return partialMatches.sort((a, b) => b.predictedConfidence - a.predictedConfidence).slice(0, 2);
  }
  ...
  async getStatistics(): Promise<any> {
    try {
      ...
      const strategyCounts = await db.select({
  ...
  async cleanOldPatterns(daysOld: number = 90): Promise<void> {
    try {
      ...
      const result = await db.delete(fieldMappingCache)
        .where(
          and(
