...
import type { UploadSession, FieldMapping, SourceField, TargetField } from '../types';
...
const ConfidenceIndicator: React.FC<{ confidence: number; showPercentage?: boolean }> = ({ 
  confidence, 
  showPercentage = true 
}) => {
...
const TARGET_FIELDS: TargetField[] = [
  ...
  { name: 'shortDescription', label: 'Short Description', type: 'string', required: false },
      ...
      switch (action) {
        case 'accept-high-confidence':
          ...
          setFieldMappings(prev => prev.map(mapping => 
            mapping.confidence >= 0.9 ? { ...mapping, isManual: true } : mapping
          ));
          toast({
        ...
        case 'auto-map':
          ...
          if (sessionData?.id) {
            ...
            toast({
              ...
              description: 'Field mappings have been updated based on AI analysis',
            });
          }
          break;
      }
      ...
      <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Map your file fields to product attributes. AI suggestions are provided with confidence scores.
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <Button 
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                ...
                <div className="text-sm text-muted-foreground">Source Fields</div>
              </div>
              <div className="text-center p-3 bg-muted/50 rounded-lg">
              ...
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                <div className="text-2xl font-bold text-green-600">{mappedRequiredFields.length}</div>
                <div className="text-sm text-muted-foreground">Required Mapped</div>
              </div>
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                <div className="text-2xl font-bold text-orange-600">{requiredFields.length - mappedRequiredFields.length}</div>
            ...
            {!canProceed && (
              <Alert className="mb-6">
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  <p className="font-medium mb-2">Missing Required Field Mappings:</p>
                  <div className="flex flex-wrap gap-2">
                    {requiredFields
                      .filter(rf => !fieldMappings.some(m => m.targetField === rf.name))
                      .map(field => (
              ...
              <Table>
                ...
                <TableBody>
                  {sourceFields.map((field) => {
                    ...
                    const isRequired = mapping && TARGET_FIELDS.find(tf => tf.name === mapping.targetField)?.required;
                      ...
                      <TableRow key={field.name}>
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            {field.sampleValues.slice(0, 3).map((value, index) => (
                            ...
                            <SelectContent>
                              ...
                              {TARGET_FIELDS.map((targetField) => (
                                <SelectItem 
                                  ...
                                  disabled={fieldMappings.some(m => 
                                    m.targetField === targetField.name && m.sourceField !== field.name
                                  )}
                                >
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button 
                                variant="ghost" 
                                size="sm"
                                onClick={() => setShowPreview(showPreview === field.name ? null : field.name)}
                              >
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    {sourceFields
                      ...
                      .map((value, index) => (
                        <div key={index} className="p-2 bg-muted/50 rounded font-mono text-xs">
                          {String(value)}
                        </div>
                      ))
                    }
            ...
            <ScrollArea className="h-40">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {TARGET_FIELDS.map((field) => (
                  <div key={field.name} className="flex items-start space-x-3 p-2">
        ...
        {canProceed && (
          <Alert>
            ...
            <AlertDescription>
              All required fields are mapped! You can proceed to the data preview step.
            </AlertDescription>
          </Alert>
        )}
      </div>
